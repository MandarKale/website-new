  ...
  return (
    ...
    >
      <div className="container mx-auto flex justify-between items-center">
        {/* Logo */}
        <Link href="/" className="text-xl font-heading font-bold text-accent dark:text-white">
          Mandar Kale
          <span className="block text-xs text-gray-600 dark:text-gray-300 font-normal mt-0.5">Engineering Leader | Vibe Coder</span>
        </Link>
        ...
        >
          <svg 
            className="w-6 h-6 text-accent dark:text-gray-300" 
            fill="none" 
            strokeLinecap="round" 
      ...
      {links.map((link) => (
        link.external ? (
          <a
            ...
            className={`text-gray-700 hover:text-accent dark:text-gray-200 dark:hover:text-white font-medium ${
              mobile ? 'text-lg' : ''
            }`}
          >
            {link.label}
    ...
    <>
      {socialLinks.map((link) => (
        <a
          ...
          target="_blank"
          rel="noopener noreferrer"
          className="text-accent hover:text-accent-dark dark:text-gray-300 dark:hover:text-white"
